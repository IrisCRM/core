// CalendarDateSelect version 1.10.11(modified by miv) - a prototype based date picker
...
CalendarDateSelect.prototype = {
  ...
  initCalendarGrid: function() {
    ...
    alert(body_div.innerHTML);
	*/
    // -----
	
    var body_div_html  = '<table cellpadding="0" cellspacing="0" width="100%"><thead><tr>';
    Date.weekdays.each( function(weekday) { 
        body_div_html += '<th>'+weekday+'</th>';
    });  
    body_div_html += '</tr></thead><tbody>';
    // Make the days!
    var row_number = 0, weekday;
    for(var cell_index = 0; cell_index<42; cell_index++) {
      weekday = (cell_index+Date.first_day_of_week ) % 7;
      if (cell_index%7 ==0) {
		body_div_html += '<tr class="row_'+row_number+'">';
		row_number++;
	  }
      ...
      if ((weekday==0) || (weekday==6))
	    ...
	    td_class = "";
      body_div_html += '<td class="'+td_class+'" style="cursor: pointer"><div></div></td>';
    }  
    body_div_html += '</tbody></table>';
	body_div.innerHTML = body_div_html;

	var row_index = 0; // miv
	var cell_index = 0; // miv
    for(var cell_index = 0; cell_index<42; cell_index++) {
      weekday = (cell_index+Date.first_day_of_week ) % 7;
	  if (cell_index % 7==0) {
        cell2_index = 0;
	  ...
	  cell_td = $(this.body_div.getElementsByTagName("table")[0].rows[row_index].cells[cell2_index++]);
	  this.calendar_day_grid[cell_index] = cell_td;
	  cell_td.calendar_date_select = this;
      cell_td.onmouseover = function () { this.calendar_date_select.dayHover(this); };
      cell_td.onmouseout = function () { this.calendar_date_select.dayHoverOut(this) };
      cell_td.onclick = function() { this.calendar_date_select.updateSelectedDate(this, true); };
    }
    // -----
  },
  ...
  updateValue: function() {
    ...
    if (last_value!=this.target_element.value) {
      ...
      //TODO: вынести это изменение в дочерний класс
      jQuery('#' + this.target_element.getAttribute('id')).trigger("change");
      jQuery('#' + this.target_element.getAttribute('id')).trigger("field:edit");
      jQuery('#' + this.target_element.getAttribute('id')).trigger("field:edited");
    }
