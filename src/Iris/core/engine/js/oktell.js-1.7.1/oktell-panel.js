...
(function (factory) {
    ...
    jQuery.event.special.oktellPanelMousewheel = {
        setup: function() {
...
jQuery.event.special.oktellPanelMwheelIntent = {
	setup: function(){
		var jElm = jQuery(this).bind('oktellPanelMousewheel', jQuery.event.special.oktellPanelMwheelIntent.handler);
	...
	teardown: function(){
        jQuery(this)
			.unbind('oktellPanelMousewheel', jQuery.event.special.oktellPanelMwheelIntent.handler)
    ...
    handler: function(e, d){
		...
		if( this === mwheelI.elem || Math.abs(mwheelI.pos[0] - pos[0]) > minDif || Math.abs(mwheelI.pos[1] - pos[1]) > minDif ){
			...
			e = jQuery.extend({}, e, {type: 'oktellPanelMwheelIntent'});
...
jQuery(function(){
	...
	jQuery(doc).bind('oktellPanelMwheelIntent.mwheelIntentDefault', jQuery.noop);
});
})(jQuery);
...
(function(jQuery,window,undefined){
	...
	jQuery.fn.oktellPanelJScrollPane = function(settings)
	{
		...
		function JScrollPane(elem, s)
		{
			var settings, jsp = this, pane, paneWidth, paneHeight, container, contentWidth, contentHeight,
				...
				mwEvent = jQuery.fn.oktellPanelMwheelIntent ? 'oktellPanelMwheelIntent.jsp' : 'oktellPanelMousewheel.jsp';
		...
		// Pluginifying code...
		settings = jQuery.extend({}, jQuery.fn.oktellPanelJScrollPane.defaults, settings);
	...
	jQuery.fn.oktellPanelJScrollPane.defaults = {
		showArrows					: false,
...
(function(jQuery) {
  ...
  if (!jQuery) {
    throw new Error('Error init oktell panel, jQuery ( jQuery ) is not defined');
  }
  ...
  Department = (function() {
    ...
    Department.prototype.getEl = function(usersVisible) {
      if (!this.el) {
        this.el = jQuery(this.template.replace(/\{\{department}\}/g, escapeHtml(this.name)));
      }
      if (usersVisible) {
  ...
  CUser = (function() {
    ...
    CUser.prototype.getEl = function(createIndependent) {
      ...
      if (!this.el || createIndependent) {
        ...
        jQueryel = jQuery(str);
        jQueryel.data('user', this);
        this.initButtonEl(jQueryel.find('.oktell_button_action'));
        this.els = this.els.add(jQueryel);
        this.setStateCss();
        if (!this.el) {
          this.el = jQueryel;
    ...
    CUser.prototype.initButtonEl = function(jQueryel) {
      var _this = this;

      this.buttonEls = this.buttonEls.add(jQueryel);
      jQueryel.data('user', this);
      jQueryel.children(':first').bind('click', function() {
        return _this.doAction(_this.buttonLastAction);
      });
      if (this.buttonLastAction) {
        return jQueryel.removeClass(this.noneActionCss).addClass(this.firstLiCssPrefix + this.buttonLastAction.toLowerCase());
      } else {
        return jQueryel.addClass(this.noneActionCss);
      }
    };
    ...
    CUser.prototype.getButtonEl = function() {
      ...
      jQueryel = jQuery(this.buttonTemplate);
      this.initButtonEl(jQueryel);
      return jQueryel;
    };
  ...
  List = (function() {
    ...
    function List(oktell, panelEl, dropdownEl, afterOktellConnect, options, contained, useSticky, useNativeScroll, debugMode) {
      ...
      this.simpleListEl = jQuery(this.usersTableTemplate);
      ...
      this.dtmfEl.find('.btn-small').bind('click', function(e) {
        return _this.sendDtmf(jQuery(e.target).text());
      });
      ...
      this.keypadEl.find('li').bind('click', function(e) {
        ...
        _this.filterInput.val(_this.filterInput.val() + jQuery(e.currentTarget).find('button').data('num'));
        return _this.filterInput.keyup();
      });
      this.setUserListHeight = function() {
        var h;

        h = jQuery(window).height() - _this.usersListBlockEl[0].offsetTop + 'px';
    ...
    List.prototype.initStickyHeaders = function() {
      ...
      if (this.headerEls.length === 0) {
        return;
      } else if (jQuery(this.headerEls[0]).width() === 0) {
        setTimeout(function() {
    ...
    List.prototype.processStickyHeaders = function(elIndex) {
      ...
      if (((_ref = this.headerEls) != null ? _ref.length : void 0) > 0) {
        if (elIndex != null) {
          ...
          jQuery('.b_sticky_header_container').empty().append(this.currentTopHeaderClone);
    ...
    List.prototype.addEventListenersForButton = function(user, button) {
      ...
      return button.bind('click', function() {
        user;        if (user) {
          return _this.showDropdown(user, jQuery(_this));
        }
      });
    };
    ...
    List.prototype.showDropdown = function(user, buttonEl, actions, onPanel) {
      ...
      if (actions != null ? actions.length : void 0) {
        ...
        if (aEls.length) {
          ...
          this.dropdownEl.css({
            'top': this.dropdownEl.height() + buttonEl.offset().top > jQuery(window).height() ? jQuery(window).height() - this.dropdownEl.height() - this.dropdownPaddingBottomLeft : buttonEl.offset().top,
    ...
    List.prototype.syncAbonentsAndUserlist = function(abonents, userlist) {
      ...
      jQuery.each(abonents, function(i, ab) {
        var number, u, _ref2;
    ...
    List.prototype._setUsersHtml = function(usersArray, jQueryel, useIndependentCopies) {
      ...
      jQueryel.children().detach();
      return jQueryel.html(html);
    };
    ...
    List.prototype.setFilter = function(filter, reloadAnyway, notScrollTop) {
      ...
      if (allDeps.length > 0) {
        jQuery(allDeps[allDeps.length - 1]).find('tr:last').addClass('g_last');
      }
  ...
  Popup = (function() {
    ...
    Popup.prototype.setAbonents = function(abonents) {
      ...
      return jQuery.each(abonents, function(i, abonent) {
  ...
  initPanel = function(opts) {
    ...
    panelEl = jQuery(panelHtml);
    if (!getOptions().showAvatar) {
      panelEl.addClass('noavatar');
    }
    jQueryuser = jQuery(userTemplateHtml);
    jQueryuserActionButton = jQuery(actionButtonHtml);
    oldBinding = jQueryuserActionButton.attr('data-bind');
    jQueryuserActionButton.attr('data-bind', oldBinding + ', visible: jQuerydata.actionBarIsVisible');
    jQueryuser.find('td.b_contact_title').append(jQueryuserActionButton);
    actionListEl = jQuery(actionListHtml);
    jQuery('body').append(actionListEl);
    oktell = getOptions().oktell;
    CUser.prototype.formatPhone = oktell.formatPhone;
    ringtone = null;
    if (getOptions().ringtone) {
      ringtone = jQuery('<audio src="' + getOptions().ringtone + '" id="oktell_panel_ringtone" preload="auto"></audio>')[0];
      jQuery("body").append(ringtone);
      ringtone.loop = true;
    }
    if (!getOptions().withoutCallPopup) {
      ...
      popupEl = jQuery(popupHtml);
      jQuery('body').append(popupEl);
    ...
    if (!getOptions().withoutPermissionsPopup) {
      ...
      permissionsPopupEl = jQuery(permissionsPopupHtml);
      jQuery('body').append(permissionsPopupEl);
    ...
    if (!useContainer) {
      ...
      jQuery(window).bind('touchcancel', function(e) {
        return true;
      });
      ...
      return jQuery('html').bind('mousemove', function(e) {
  ...
  initActionButtons = function(selector) {
    return jQuery(selector + ":not(." + actionButtonContainerClass + ")").each(function() {
      return addActionButtonToEl(jQuery(this));
    });
  };
  jQuery.oktellPanel = function(arg) {
  ...
  jQuery.fn.oktellButton = function() {
    return jQuery(this).each(function() {
      return addActionButtonToEl(jQuery(this));
    });
  };
  jQuery.fn.oktellPanel = function() {
    var args;

    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    if (!panelWasInitialized) {
      args.container = jQuery(this);
      return jQuery.oktellPanel.apply(window, args);
    } else if (jQuery(this)[0] === (panelEl != null ? panelEl[0] : void 0)) {
      return jQuery.oktellPanel.apply(window, args);
    }
  };
  jQuery.oktellPanel.show = function() {
    return list.showPanel();
  };
  return jQuery.oktellPanel.hide = function() {
