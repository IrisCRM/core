	...
	(function (JsSIP) {
		...
		var NonInviteClientTransactionPrototype = function () {
			this.send = function () {
				...
				this.F = window.setTimeout(function () {
					tr.timer_F();
				...
				if (status_code < 200) {
					...
					switch (this.state) {
						...
						case C.STATUS_PROCEEDING:
							...
							this.K = window.setTimeout(function () {
								tr.timer_K();
							}, JsSIP.Timers.TIMER_K);
							break;
				...
				if (status_code > 100 && status_code <= 199 && this.state === C.STATUS_PROCEEDING) {
					...
					if (this.resendProvisionalTimer === null) {
						this.resendProvisionalTimer = window.setInterval(function () {
							tr.resend_provisional();
						}, JsSIP.Timers.PROVISIONAL_RESPONSE_INTERVAL);
					}
				} else if (status_code >= 200 && status_code <= 299) {
					switch (this.state) {
						case C.STATUS_PROCEEDING:
				...
				case JsSIP.C.ACK:
					...
					if (tr) {
						if (tr.state === C.STATUS_ACCEPTED) {
							...
							tr.I = window.setTimeout(function () {
								tr.timer_I();
							}, JsSIP.Timers.TIMER_I);
							return true;
						}
					}
...
// Load dependencies
		var RequestSender = (function (JsSIP) {
		...
		var RTCMediaHandler = (function (JsSIP) {
...
//						} else {
//							console.log(LOG_PREFIX + 'ICE complete');
							if ( self && typeof self.onIceCompleted == 'function'){
								self.onIceCompleted();
							}
//						}
					};
...
//							this.localMedia.stop();
				...
				getUserMedia: function (ua,onSuccess, onFailure, constraints) {
					...
					var okMediaStream = oktellVoice && typeof oktellVoice.getUserMediaStream == 'function' ? oktellVoice.getUserMediaStream() : undefined;
					if ( okMediaStream ){
						onSuccessGetUserMedia(okMediaStream);
					} else {
						if ( oktellVoice && typeof oktellVoice.getUserMediaStream == 'function' ) {
							ua.emit('mediaPermissionsRequest', ua);
							oktellVoice.getUserMediaStream(onSuccessGetUserMedia, onError, false);
						} else {
		...
		UA.prototype.loadConfig = function (configuration) {
			...
			for (parameter in UA.configuration_check.optional) {
				if (configuration.hasOwnProperty(parameter)) {
					...
					else if (typeof(value) === 'number' && window.isNaN(value)) {
						continue;
					}
			...
			this.predefinedStream = settings.predefinedStream;
	...
	var result = {
		...
		parse: function (input, startRule) {
			...
			function parse_unreserved() {
				...
				if (result0 === null) {
					result0 = parse_mark(true); // with #
				}
				return result0;
			}

			function parse_mark(withHash) {
				var result0;
